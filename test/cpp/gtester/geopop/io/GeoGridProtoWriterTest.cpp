...
TEST(GeoGridProtoWriterTest, contactPoolsTest)
{
        ...
        loc->GetContent()->RefPools(Id::Daycare).emplace_back(pop->RefPoolSys().CreateContactPool(Id::Daycare));
        loc->GetContent()->RefPools(Id::PreSchool).emplace_back(pop->RefPoolSys().CreateContactPool(Id::PreSchool));
        loc->GetContent()->RefPools(Id::K12School).emplace_back(pop->RefPoolSys().CreateContactPool(Id::K12School));
        loc->GetContent()
            ->RefPools(Id::PrimaryCommunity)
            .emplace_back(pop->RefPoolSys().CreateContactPool(Id::PrimaryCommunity));
