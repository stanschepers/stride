...
void MapController::initialize(QObject *root) {
    ...
    for( auto const& location : m_epiOutput )
    {
        ...
        QMetaObject::invokeMethod(root, "addLocation", Q_ARG(QVariant, id), Q_ARG(QVariant, QVariant::fromValue(location.latitude)),
                                  Q_ARG(QVariant, QVariant::fromValue(location.longitude)), Q_ARG(QVariant, QVariant::fromValue(location.pop_count * 100)));
    }

    centerLatitude /= m_epiOutput.size();
    centerLongitude /= m_epiOutput.size();
    QMetaObject::invokeMethod(root, "initialize", Q_ARG(QVariant, QVariant::fromValue(zoomlevel)), Q_ARG(QVariant, QVariant::fromValue(centerLatitude)),
                              Q_ARG(QVariant, QVariant::fromValue(centerLongitude)), Q_ARG(QVariant, QVariant::fromValue(firstDay)), Q_ARG(QVariant, QVariant::fromValue(lastDay)));
