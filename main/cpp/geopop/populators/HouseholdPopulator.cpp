...
void Populator<stride::ContactType::Id::Household>::Apply(GeoGrid<Epidemiologic>& geoGrid, const GeoGridConfig& geoGridConfig)
{
        ...
        if (geoGridConfig.refHHperHHType.empty()) {
                ...
                for (const shared_ptr<Location<Epidemiologic>>& loc : geoGrid) {
                        auto hh_dist = genPerHhType.at(loc->GetContent()->GetHouseHoldType());
                        for (auto& pool : loc->GetContent()->RefPools(Id::Household)) {
                                const auto hDraw = static_cast<unsigned int>(hh_dist());
                                for (const auto& age : geoGridConfig.refHHperHHType.at(loc->GetContent()->GetHouseHoldType()).ages[hDraw]) {
                                        const auto p = pop->CreatePerson(person_id++, age, pool->GetId(), 0, 0, 0, 0, 0, 0, 0);
                                        pool->AddMember(p);
                                }
                        }
