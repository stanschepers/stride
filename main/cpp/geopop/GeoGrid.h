...
public:
        ...
        void AddLocation(std::shared_ptr<Location<LocationContent>> location);

        /// Disables the addLocation method and builds the kdtree.
        void Finalize();

        /// Gets a Location by Id and check if the Id exists.
        std::shared_ptr<Location<LocationContent>> GetById(unsigned int id) const { return m_locations[m_id_to_index.at(id)]; }
        ...
        std::vector<const Location<LocationContent>*> LocationsInRadius(const Location<LocationContent>& start, double radius) const;
         ...
         *  +-------p2    p2------+
         */
        std::set<const Location<LocationContent>*> LocationsInBox(double long1, double lat1, double long2, double lat2) const;

        /// Gets the location in a rectangle defined by the two Locations.
        std::set<const Location<LocationContent>*> LocationsInBox(Location<LocationContent>* loc1, Location<LocationContent>* loc2) const;

        /// Gets the K biggest (in population count) locations of this GeoGrid
        std::vector<Location<LocationContent>*> TopK(size_t k) const;
...
public:
        using iterator       = typename std::vector<std::shared_ptr<Location<LocationContent>>>::iterator;
        using const_iterator = typename std::vector<std::shared_ptr<Location<LocationContent>>>::const_iterator;
        ...
        std::shared_ptr<Location<LocationContent>>& operator[](size_t index) { return m_locations[index]; }

        /// Gets a Location by index, doesn't performs a range check.
        const std::shared_ptr<Location<LocationContent>>& operator[](size_t index) const { return m_locations[index]; }
...
private:
        ...
        std::vector<std::shared_ptr<Location<LocationContent>>> m_locations;
